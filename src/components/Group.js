...
const Group = ({ students, group, displayRating, baseRating }) => {
    return (
        <div className="card" style={{ boxShadow: "1px 1px 10px 0px #80808070", marginBottom: "24px" }}>
            <div className="card-body">
                <h5 className="card-title">Groupe {group.id}: {group.subject}</h5>
                    ...
                    <ul className="list-group">
                        {students.map(student => (
                            <li className="list-group-item" key={`${group.name}-${student.name}`}>
                                <Student student={student} baseRating={baseRating} displayRating={displayRating} />
                            </li>
                        ))}
                    </ul>
                }
